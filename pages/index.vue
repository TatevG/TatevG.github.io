  ...
  methods: {
    async searchAddressByQuery() {
        ...
        try {
          ...
          this.searchResults = searchResults.features.map(element => {
            return { name: element.properties.name, country: element.properties.country }
          });
        } catch (error) {
          console.log(error);
        }
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
          if(position.coords.latitude && position.coords.longitude) {
            ...
            const response = await GetWeatherByCoordinates(position.coords.latitude, position.coords.longitude);
            if(!response) return;
            this.locationData = response.data[0];
            this.location = response.data[0].name;
            this.searchResults.push({ name: response.data[0].name, country: response.data[0].country })
    ...
    async getPastDaysData(lat, lon) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        try {
          const response = await GetPastDaysData(lat, lon, day);
